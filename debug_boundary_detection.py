...
def debug_boundary_detection():
    ...
    while True:
        ...
        # 降低Canny阈值，检测更多边缘
        edges_canny_low = cv2.Canny(gray, 30, 100)  # 原来: 80, 200
        edges_canny_high = cv2.Canny(gray, 80, 200)  # 原来的阈值
        
        cv2.imshow('Canny Low Threshold (30,100)', edges_canny_low)
        cv2.imshow('Canny High Threshold (80,200)', edges_canny_high)
        
        # 4. 放宽条件的轮廓检测
        contours_low, _ = cv2.findContours(edges_canny_low, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
        contours_high, _ = cv2.findContours(edges_canny_high, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
        ...
        # 6. 打印统计信息
        print(f"\r检测到轮廓数量 - 低阈值: {len(contours_low)}, 高阈值: {len(contours_high)}", end="")
