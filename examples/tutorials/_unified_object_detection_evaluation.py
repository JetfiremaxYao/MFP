...
# 导入两种检测方法
from _find_the_object_hsv_depth import (
...
class UnifiedObjectDetectionEvaluator:
    ...
    def _analyze_results(self):
        ...
        # 转换为DataFrame便于分析
        df = pd.DataFrame([vars(result) for result in self.trial_results])
    ...
    def _save_results(self):
        ...
        results_dict = {
            ...
            'trial_results': [vars(result) for result in self.trial_results]
        }
    ...
    def _generate_visualization(self):
        ...
        try:
            ...
            axes[0, 0].set_title('位置误差对比')
            axes[0, 0].set_ylabel('位置误差 (m)')
            axes[0, 0].set_xlabel('光照条件')
            
            # 2. 范围误差箱线图
            sns.boxplot(data=df, x='lighting', y='range_err_m', hue='method', ax=axes[0, 1])
            axes[0, 1].set_title('范围误差对比')
            axes[0, 1].set_ylabel('范围误差 (m)')
            axes[0, 1].set_xlabel('光照条件')
            
            # 3. 检测时间箱线图
            sns.boxplot(data=df, x='lighting', y='detection_time_s', hue='method', ax=axes[0, 2])
            axes[0, 2].set_title('检测时间对比')
            axes[0, 2].set_ylabel('检测时间 (s)')
            axes[0, 2].set_xlabel('光照条件')
            ...
            axes[1, 0].set_title('成功率对比 (2cm阈值)')
            axes[1, 0].set_ylabel('成功率')
            axes[1, 0].set_xlabel('光照条件')
