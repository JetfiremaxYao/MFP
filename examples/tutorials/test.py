...
try:
    ...
    print("警告：PyTorch未安装，HED和SE方法将不可用")
    DL_AVAILABLE = False
...
try:
    ...
    print("警告：SciPy未安装，某些高级滤波功能将不可用")
...
4. hed: Holistically-Nested Edge Detection
   - 优点：基于深度学习，对复杂场景鲁棒，精度高
   - 适用：复杂光照和阴影条件下的边界检测
   - 缺点：需要GPU加速，计算量较大

5. se: Structure Edge Detection
   - 优点：结构化边缘检测，抗噪声能力强
   - 适用：需要精确结构边缘的场景
   - 缺点：需要预训练模型
...
def detect_object_boundary(cam, method='rgbd_hybrid', debug_vis=False):
    ...
    cam : Camera
        相机对象
    method : str
        检测方法: 'canny', 'hsv', 'hybrid', 'adaptive', 'rgbd_depth', 'rgbd_hybrid', 'rgbd_3d', 'hed', 'se'
    debug_vis : bool
        是否显示调试图像
    ...
    if method == 'canny':
        ...
        contours, edges = _detect_boundary_hed(img)
    elif method == 'se':
        # Structure Edge Detection
        contours, edges = _detect_boundary_se(img)
    else:
...
def _detect_boundary_hed(img):
    """
    Holistically-Nested Edge Detection (HED)
    基于深度学习的边缘检测方法
    """
    if not DL_AVAILABLE:
        print("警告：PyTorch未安装，回退到Canny方法")
        return _detect_boundary_canny(img)
    
    try:
        # 简化的HED实现（使用预训练的VGG特征）
        # 在实际应用中，你需要下载预训练的HED模型
        ...
        # 使用简化的边缘检测网络
        edges = _hed_inference(img_tensor)
        ...
        contours, _ = cv2.findContours(edges_binary, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
        return contours, edges_binary
        
    except Exception as e:
        print(f"HED检测失败: {e}，回退到Canny方法")
        return _detect_boundary_canny(img)

def _detect_boundary_se(img):
    """
    Structure Edge Detection (SE)
    基于结构化森林的边缘检测方法
    """
    if not DL_AVAILABLE:
        print("警告：PyTorch未安装，回退到Canny方法")
    ...
    try:
        ...
        # 使用简化的结构化边缘检测
        edges = _se_inference(img_tensor)
        
        # 后处理
        edges = (edges * 255).astype(np.uint8)
        
        # 自适应阈值
        median_val = np.median(edges)
        lower = int(max(0, (1.0 - 0.33) * median_val))
        upper = int(min(255, (1.0 + 0.33) * median_val))
        
        _, edges_binary = cv2.threshold(edges, lower, upper, cv2.THRESH_BINARY)
        ...
        print(f"SE检测失败: {e}，回退到Canny方法")
        return _detect_boundary_canny(img)

def _hed_inference(img_tensor):
    """
    HED推理（简化版本）
    在实际应用中，这里应该加载预训练的HED模型
    """
    # 简化的HED实现：使用VGG特征提取
    with torch.no_grad():
        # 这里应该使用真正的HED模型
        ...
        edges = torch.sigmoid(edges)  # 归一化到0-1
        
        return edges.squeeze().cpu().numpy()

def _se_inference(img_tensor):
    """
    SE推理（简化版本）
    在实际应用中，这里应该加载预训练的SE模型
    """
    # 简化的SE实现：使用多尺度特征
    with torch.no_grad():
        # 多尺度处理
        ...
        for scale in scales:
            if scale != 1.0:
                ...
                scaled_img = F.interpolate(img_tensor, size=(new_h, new_w), mode='bilinear', align_corners=False)
            else:
                scaled_img = img_tensor
            
            # 简化的结构化边缘检测
            gray = torch.mean(scaled_img, dim=1, keepdim=True)
            
            # 使用更大的核进行边缘检测
            kernel = torch.ones(5, 5, dtype=torch.float32).view(1, 1, 5, 5) / 25
            smoothed = F.conv2d(gray, kernel, padding=2)
            
            # 计算梯度
            grad_x = torch.diff(smoothed, dim=3, prepend=smoothed[:, :, :, :1])
            grad_y = torch.diff(smoothed, dim=2, prepend=smoothed[:, :, :1, :])
            ...
            # 恢复到原始尺寸
            if scale != 1.0:
                edges = F.interpolate(edges, size=(img_tensor.shape[2], img_tensor.shape[3]), 
                                    mode='bilinear', align_corners=False)
            
            edge_maps.append(edges)
        
        # 融合多尺度结果
        combined_edges = torch.mean(torch.stack(edge_maps), dim=0)
        return combined_edges.squeeze().cpu().numpy()
...
def test_boundary_detection_methods(scene, ed6, cam, motors_dof_idx, j6_link, cube_pos):
    ...
    methods = ['canny', 'hsv', 'hybrid', 'adaptive', 'rgbd_depth', 'rgbd_hybrid', 'rgbd_3d', 'hed', 'se']
    current_pos_idx = 0
    ...
    while True:
        ...
        if choice == "1":
            ...
            try:
                method_idx = int(input("请选择方法编号 (1-9): ")) - 1
...
def main():
    ...
    cube = scene.add_entity(gs.morphs.Box(size=(0.105, 0.18, 0.022), pos=(0.3, 0, 0.02), collision=True))
