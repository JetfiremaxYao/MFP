...
try:
    while True:
        ...
        j6_quat = j6_link.get_quat().cpu().numpy()
        r = R.from_quat([j6_quat[1], j6_quat[2], j6_quat[3], j6_quat[0]])
        z_axis_world = r.apply([0, 0, 1])   # J6末端z轴在世界坐标系下的方向
        y_axis_world = r.apply([0, 1, 0])   # J6末端y轴（可作为相机up向量）
        cam.set_pose(
            pos=j6_pos,
            lookat=j6_pos + z_axis_world,  # 镜头朝z轴正方向
            up=y_axis_world,                # up向量与J6末端y轴一致
        )
        # 渲染相机图像
        rgb, depth, segmentation, normal = cam.render(
            rgb=True, depth=True, segmentation=True, normal=True
