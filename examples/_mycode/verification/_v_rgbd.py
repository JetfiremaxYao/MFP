...
def track_and_scan_boundary_jump(scene, ed6, cam, motors_dof_idx, j6_link, cube_pos, jump_height=0.1, max_steps=40):
    ...
    for step in range(max_steps):
        ...
        o3d.visualization.draw_geometries([pcd], window_name=f"Step {step+1} Boundary PointCloud (RGB-D)", width=400, height=300)
    ...
    print(f"跳跃式边界扫描完成 (RGB-D方法)，总共采集到{len(all_boundary_points)}个边界点")
...
def main():
    ...
    cube = scene.add_entity(gs.morphs.Mesh(
        file="genesis/assets/_myobj/ttt.obj",
        pos=(0.30, 0.05, 0.02),
        collision=True,
        fixed=True,
    ))
    ...
    # 保存边界点云
    visualize_and_save_pointcloud(boundary_points, "boundary_cloud_rgbd.ply")
    
    # 保存表面点云
    if len(surface_points) > 0:
        visualize_and_save_pointcloud(surface_points, "surface_cloud_rgbd.ply")
...
def detect_surface_anomalies(surface_grid, grid_coords, sigma_scales=[1.0, 2.0], 
    ...
    print(f"表面高度统计: 均值={mean_height:.4f}, 标准差={std_height:.4f}")
    print(f"高度范围: {min_height:.4f} - {max_height:.4f} (差值: {(max_height-min_height)*1000:.1f}mm)")
    
    # 改进的阈值计算：使用标准差作为基准
    # 方法1：基于标准差的阈值
    std_threshold = mean_height + 1.5 * std_height  # 1.5倍标准差
    
    # 方法2：基于百分位数的阈值（降低到75%提高敏感度）
    ...
    if len(valid_diff) > 0:
        # 使用更低的百分位数，提高检测敏感度
        threshold = np.percentile(valid_diff, 60)  # 降低到60%
    else:
        threshold = 0.001  # 默认1mm阈值
    ...
    for i in range(1, num_anomalies + 1):
        ...
        anomaly_info = {
            ...
            'relative_strength': relative_strength,  # 新增：相对强度
...
def generate_palpation_suggestions(anomalies, cube_pos, surface_points=None, max_suggestions=3):
    """
    生成触诊建议位置 - 直接基于表面点云最高点
    
    Parameters:
    -----------
    anomalies : list
        检测到的异常区域列表（未使用，保持接口一致）
    cube_pos : np.ndarray
        物体中心位置
    surface_points : np.ndarray
        表面点云数据
        ...
        if max_suggestions > 1:
            ...
            for i in range(1, min(max_suggestions, 4)):  # 最多3个额外建议
                ...
                if len(selected_positions) > 0:
                    # 修复广播错误：计算到所有已选择点的最小距离
                    selected_positions = np.array(selected_positions)  # 形状: (n_selected, 2)
                    remaining_xy = remaining_points[:, :2]  # 形状: (n_remaining, 2)
                    
                    # 计算每个剩余点到所有已选择点的距离
                    # 使用广播：remaining_xy[:, None, :] - selected_positions[None, :, :]
                    # 结果形状: (n_remaining, n_selected, 2)
                    diff = remaining_xy[:, None, :] - selected_positions[None, :, :]
                    distances_to_all = np.linalg.norm(diff, axis=2)  # 形状: (n_remaining, n_selected)
                    
                    # 对每个剩余点，找到到最近已选择点的距离
                    min_distances = np.min(distances_to_all, axis=1)  # 形状: (n_remaining,)
