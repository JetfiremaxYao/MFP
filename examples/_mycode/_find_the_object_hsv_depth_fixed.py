...
def detect_cube_position(scene, ed6, cam, motors_dof_idx, 
    ...
    if len(results) == 0:
        ...
        if lighting_condition == "bright":
            ...
            if base_view_hit_rate < 0.3:  # 如果检测率低于30%，说明光照太亮影响了检测
                lighting_adjusted_hit_rate = base_view_hit_rate * 0.8  # 降低召回率
            else:
                lighting_adjusted_hit_rate = base_view_hit_rate
        elif lighting_condition == "dim":
            # 昏暗光照：HSV检测应该更困难，检测率低是正常的
            if base_view_hit_rate < 0.2:  # 如果检测率低于20%，说明光照太暗
                lighting_adjusted_hit_rate = base_view_hit_rate * 0.7  # 进一步降低召回率
            else:
