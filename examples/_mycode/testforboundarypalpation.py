...
def track_and_scan_boundary_jump(scene, ed6, cam, motors_dof_idx, j6_link, cube_pos, jump_height=0.1, max_steps=30):
    ...
    for step in range(max_steps):
        ...
        # 使用高密度采样策略突出表面细节
        sample_rate = 0.10  # 采样15%的点（从5%提升到15%）
        for py in range(0, depth.shape[0], 2):  # 每隔2个像素采样（从3个提升到2个）
            for px in range(0, depth.shape[1], 2):  # 每隔2个像素采样（从3个提升到2个）
                if not boundary_mask[py, px] and depth[py, px] > 0.05:  # 排除边界点和无效深度
                    # 检查是否在物体表面范围内
        ...
        if len(surface_points) > 0:
            ...
            print(f"第{step+1}步，采集到{len(surface_points)}个表面点（采样率10%）")
        
        # 11. 闭环检测
        if step >= 2 and step - last_closure_check_step >= closure_check_interval and len(all_points) > 0:
...
def visualize_surface_analysis(surface_grid, grid_coords, anomalies, suggestions, 
    ...
    ax.set_box_aspect([x_range, y_range, z_range * 0.8])
