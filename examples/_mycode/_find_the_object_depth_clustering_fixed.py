...
def detect_cube_position(scene, ed6, cam, motors_dof_idx, 
    ...
    print(f"光照条件: {lighting_condition}, 使用深度聚类检测")
    ...
    for i, angle in enumerate(angles):
        ...
        cv2.putText(depth_colored, f"Lighting: {lighting_condition}", (10, 60), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
            ...
            if center_cam[2] > 0:
                ...
                if 0 <= px < w and 0 <= py < h:
                    ...
                    cv2.putText(img_vis, f"Points: {len(cluster_points)}", (10, 60), 
                              cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
                    cv2.putText(img_vis, f"Lighting: {lighting_condition}", (10, 90), 
                              cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
                ...
                cv2.putText(img_vis, f"Lighting: {lighting_condition}", (10, 60), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                # Debug visualization removed
    ...
    if len(results) == 0:
        ...
        # 2. 根据光照条件调整召回率
        if lighting_condition == "bright":
            # 明亮光照：深度检测应该更稳定，检测率应该较高
            if base_view_hit_rate < 0.4:  # 如果检测率低于40%，说明有问题
                lighting_adjusted_hit_rate = base_view_hit_rate * 0.85  # 降低召回率
            else:
                lighting_adjusted_hit_rate = base_view_hit_rate
        elif lighting_condition == "dim":
            # 昏暗光照：深度检测可能不稳定，检测率低是正常的
            if base_view_hit_rate < 0.3:  # 如果检测率低于30%，说明光照太暗
                lighting_adjusted_hit_rate = base_view_hit_rate * 0.75  # 进一步降低召回率
            else:
                lighting_adjusted_hit_rate = base_view_hit_rate * 0.9  # 稍微降低召回率
        else:  # normal
            # 正常光照：使用基础召回率
            lighting_adjusted_hit_rate = base_view_hit_rate
...
def main():
    ...
    plane = scene.add_entity(gs.morphs.Plane(collision=True))iva
    ...
    print("=== Depth+Clustering 物体检测方法 (修复版) ===")
