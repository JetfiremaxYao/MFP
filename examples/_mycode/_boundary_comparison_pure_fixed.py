...
try:
    ...
    from shapely.ops import unary_union  # 新版本使用unary_union替代cascaded_union
    from scipy.spatial import Delaunay
    from scipy.spatial.distance import pdist, squareform
    ALPHA_SHAPE_AVAILABLE = True
    print("✅ Alpha-Shape相关库已安装，将支持Alpha-Shape方法")
except ImportError as e:
    ALPHA_SHAPE_AVAILABLE = False
    print(f"警告: Alpha-Shape相关库未安装，将跳过Alpha-Shape方法: {e}")
...
def detect_boundary_alpha_shape(cam, alpha_value=None, min_contour_area=100):
    ...
    try:
        ...
        alpha_shape_pixels = compute_alpha_shape_pixels_simple(edge_pixels, alpha_value)
...
def compute_alpha_shape_pixels_simple(pixels, alpha=None):
    """简化的像素坐标系Alpha-Shape计算"""
    if len(pixels) < 3:
        return np.array([])
    ...
    try:
        # 计算自适应Alpha参数
        if alpha is None:
            alpha = compute_adaptive_alpha_simple(pixels)
...
def compute_adaptive_alpha_simple(pixels):
    """简化的自适应Alpha参数计算"""
    if len(pixels) < 2:
        return 1.0
...
def compute_alpha_shape_safe(points, alpha=None):
    """计算Alpha-Shape（凹包）- 安全版本"""
    if len(points) < 3:
        return np.array([])
    
    # 确保点是2D的
    if len(points.shape) != 2 or points.shape[1] != 2:
        print(f"警告：点云维度错误，期望2D，实际{points.shape}")
        return np.array([])
    
    try:
        if alpha is None:
            alpha = compute_optimal_alpha_safe(points)
...
def compute_alpha_shape(points, alpha=None):
    ...
    # 确保点是2D的
    if points.shape[1] != 2:
        print(f"警告：点云维度错误，期望2D，实际{points.shape[1]}D")
        return np.array([])
...
def compute_optimal_alpha_safe(points):
    """自动计算最优的Alpha参数 - 安全版本"""
    if len(points) < 2:
        return 1.0
    ...
    # 确保点是2D的
    if len(points.shape) != 2 or points.shape[1] != 2:
        print(f"警告：compute_optimal_alpha_safe期望2D点，实际{points.shape}")
        return 1.0
...
def compute_optimal_alpha(points):
    ...
    # 确保点是2D的
    if points.shape[1] != 2:
        print(f"警告：compute_optimal_alpha期望2D点，实际{points.shape[1]}D")
        return 1.0
...
def order_boundary_points(points):
    ...
    # 确保点是2D的
    if points.shape[1] != 2:
        print(f"警告：order_boundary_points期望2D点，实际{points.shape[1]}D")
        return points
