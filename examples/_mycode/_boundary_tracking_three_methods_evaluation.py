...
class ThreeMethodsBoundaryTrackingEvaluator:
    ...
    def __init__(self, 
        ...
        print(f"统一测试物体: ctry.obj")
        print(f"物体尺寸: {cube_size}")
        print(f"基础位置: {base_cube_pos}")
    ...
    def _setup_scene(self, cube_pos: np.ndarray):
        ...
        # 添加统一的测试物体 - 使用ctry.obj
        cube = scene.add_entity(gs.morphs.Mesh(
            file="genesis/assets/_myobj/ctry.obj",  # 统一使用ctry.obj
            pos=cube_pos,
            collision=True,
            fixed=True,
        ))
    ...
    def _create_visualizations(self, summary_df):
        ...
        fig.suptitle('三种边界追踪方法比较分析 (统一测试物体: ctry.obj)', fontsize=16, fontweight='bold')
        ...
        # 保存图表
        chart_file = self.results_dir / "three_methods_comparison_charts.png"
    ...
    def _generate_text_report(self, summary_df):
        """生成文本报告"""
        report_file = self.results_dir / "three_methods_evaluation_report.txt"
        
        with open(report_file, 'w', encoding='utf-8') as f:
            f.write("三种边界追踪方法统一评估报告\n")
            f.write("=" * 50 + "\n\n")
            
            f.write(f"评估时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write(f"评估方法: {', '.join(summary_df['method'].tolist())}\n")
            f.write(f"统一测试物体: ctry.obj\n")
            f.write(f"物体尺寸: {self.cube_size}\n")
            ...
            # 综合评分（成功率权重0.4，速度权重0.3，轮廓数量权重0.3）
            summary_df['综合评分'] = (
                summary_df['success_rate'] * 0.4 + 
                (1 - summary_df['avg_execution_time'] / summary_df['avg_execution_time'].max()) * 0.3 +
                (summary_df['avg_contours'] / summary_df['avg_contours'].max()) * 0.3
            )
            
            best_overall = summary_df.loc[summary_df['综合评分'].idxmax(), 'method']
            f.write(f"综合评分最高: {best_overall}\n")
            f.write(f"推荐用于您的pipeline: {best_overall}\n")
