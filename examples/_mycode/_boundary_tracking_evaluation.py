...
class BoundaryTrackingEvaluator:
        ...
        self.test_objects = [
            {
                ...
                'type': 'box',
                'size': cube_size,
                'base_pos': base_cube_pos,
                'file': None
            },
            {
                'name': 'ctry.obj',
                'type': 'mesh',
                'size': None,  # 网格物体不需要size
                'base_pos': np.array([0.35, -0.08, 0.02]),  # 第二个物体位置
    ...
    def _generate_perturbed_positions(self, base_pos: np.ndarray) -> List[np.ndarray]:
        """生成扰动后的物体位置"""
        positions = []
    ...
    def _setup_scene(self, obj_config: dict, obj_pos: np.ndarray, lighting: str = "normal", background: str = "simple"):
        """
        设置实验场景
    ...
    def _run_single_experiment(self, method: str, obj_config: dict, obj_pos: np.ndarray, 
        ...
        result : Dict[str, Any]
            实验结果
        """
        print(f"\n=== 运行 {method.upper()} 方法 - {obj_config['name']} - {lighting}/{background} - 第 {run_id + 1} 次实验 ===")
        print(f"物体位置: {obj_pos}")
        print(f"光照条件: {lighting}")
        
        try:
            # 设置场景
            scene, ed6, cam, motors_dof_idx, j6_link = self._setup_scene(obj_config, obj_pos, lighting, background)
            
            # 初始化机械臂
            self._reset_arm(scene, ed6, motors_dof_idx)
            
            # 移动到目标位置
            self._move_to_target(scene, ed6, motors_dof_idx, j6_link, obj_pos, cam)
            ...
            # 运行边界扫描并获取结果
            if method == 'canny':
                points = track_canny(scene, ed6, cam, motors_dof_idx, j6_link, obj_pos)
                # 从扫描过程中获取闭环检测结果
                closure_info = self._extract_closure_info_from_logs()
            else:  # rgbd
                points = track_rgbd(scene, ed6, cam, motors_dof_idx, j6_link, obj_pos)
                closure_info = self._extract_closure_info_from_logs()
            ...
            # 评估结果
            result = self._evaluate_single_run(points, obj_config, obj_pos, method, lighting, background, run_id, execution_time)
            
            # 保存点云数据
            self._save_pointcloud(points, method, run_id)
            
            # 保存增强的可视化
            self._save_enhanced_visualization(points, method, run_id, obj_pos, result)
    ...
    def _evaluate_single_run(self, points: np.ndarray, obj_config: dict, obj_pos: np.ndarray, 
                            method: str, lighting: str, background: str, run_id: int, execution_time: float) -> Dict[str, Any]:
        """
        评估单次运行结果
        ...
        # 投影到扫描平面
        points_2d, transform_matrix = self._project_to_scanning_plane(points, plane_point=obj_pos)
        
        # 生成真值矩形 - 修复：传入当前实验的obj_pos
        rect_vertices, edge_samples = self._generate_ground_truth_rectangle(transform_matrix, obj_pos)
    ...
    def run_comparison_experiments(self):
        ...
        # 运行实验 - 遍历所有物体、光照环境和方法
        total_experiments = len(self.test_objects) * len(self.lighting_conditions) * len(self.background_conditions) * self.n_runs * 2
        current_experiment = 0
        
        for obj_config in self.test_objects:
            print(f"\n{'='*60}")
            print(f"测试物体: {obj_config['name']}")
            print(f"{'='*60}")
            ...
            for lighting in self.lighting_conditions:
                for background in self.background_conditions:
                    for run_id in range(self.n_runs):
                        ...
                        obj_pos = obj_config['base_pos'].copy()
                        obj_pos[0] += x_perturb
                        obj_pos[1] += y_perturb
                        obj_pos[2] += z_perturb
                        ...
                        canny_result = self._run_single_experiment('canny', obj_config, obj_pos, lighting, background, run_id)
                        self.results[obj_config['name']]['canny'][lighting].append(canny_result)
                        
                        # 运行RGB-D方法
                        current_experiment += 1
                        print(f"\n进度: {current_experiment}/{total_experiments}")
                        rgbd_result = self._run_single_experiment('rgbd', obj_config, obj_pos, lighting, background, run_id)
                        self.results[obj_config['name']]['rgbd'][lighting].append(rgbd_result)
                        
                        print(f"第 {run_id + 1} 次实验完成 ({obj_config['name']}/{lighting}/{background})")
    ...
    def _analyze_results(self):
        ...
        for obj_config in self.test_objects:
            ...
            for lighting in self.lighting_conditions:
                ...
                # Canny方法统计
                canny_results = self.results[obj_name]['canny'][lighting]
                ...
                # RGB-D方法统计
                rgbd_results = self.results[obj_name]['rgbd'][lighting]
    ...
    def _save_results(self):
        ...
        for obj_config in self.test_objects:
            ...
            for method in ['canny', 'rgbd']:
                for lighting in self.lighting_conditions:
                    for result in self.results[obj_name][method][lighting]:
    ...
    def _save_to_csv(self, result: Dict[str, Any]):
        ...
        csv_row = {
            # 实验基本信息
            'experiment_id': f"{result['object_name']}_{result['method']}_{result['lighting']}_{result['run_id']}",
            ...
            for obj_config in self.test_objects:
                ...
                for lighting in self.lighting_conditions:
                    ...
                    for method in ['canny', 'rgbd']:
                        ...
                        for i, result in enumerate(self.results[obj_name][method][lighting]):
...
class EnhancedVisualization:
    ...
    def __init__(self, results_dir: Path):
        ...
        self.colors = {'canny': '#1f77b4', 'rgbd': '#ff7f0e'}
...
def main():
    ...
    evaluator = BoundaryTrackingEvaluator(
        ...
        experiment_name="multi_object_boundary_tracking_comparison"
    )
