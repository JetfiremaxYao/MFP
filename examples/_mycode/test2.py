...
def depth_based_clustering_detection(depth_image, cam, 
    ...
    # 5. === 关键修正1: 地面剔除 ===
    # 过滤掉地面点（z < 0.005米，即5mm）
    ground_filter_mask = points_world[:, 2] > 0.005
...
def detect_cube_position(scene, ed6, cam, motors_dof_idx, 
    ...
    # 深度聚类参数 - 针对单一规则物体优化
    min_depth = 0.15  # 最小深度阈值（米）
    max_depth = 0.6   # 最大深度阈值（米）
    eps = 0.02        # DBSCAN邻域半径（米单位，2cm）
    ...
    for i, angle in enumerate(angles):
        ...
        # 停止逻辑：连续一次未检测到物体且已经检测到过物体（与HSV方法保持一致）
    ...
    # 选择最佳检测结果（聚类大小最大，距离中心最近）
    best = max(results, key=lambda r: (r['cluster_size'], -r['dist_to_center']))
    ...
    time.sleep(2)  # 减少等待时间
    return cube_pos, detected_range
